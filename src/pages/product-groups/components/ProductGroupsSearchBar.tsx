/**
 * Search and filter bar component for Product Groups page
 */

import { SingleSelect } from '@/components/SingleSelect'

interface ProductGroupsSearchBarProps {
  searchQuery: string
...
export function ProductGroupsSearchBar({
  searchQuery,
  onSearchChange,
  onClearSearch,
  statusFilter,
    ...
    <div className="mb-6 flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
      ...
      {hasActiveFilters && (
        <div className="text-sm text-muted-foreground">
          Showing {filteredCount} of {totalCount} product group{totalCount !== 1 ? 's' : ''}
        </div>
      )}
    </div>
  )
